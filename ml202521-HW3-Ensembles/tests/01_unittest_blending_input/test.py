import numpy as np
from solution import BlendingClassifier
from sklearn.metrics import roc_auc_score, accuracy_score

def test_binary():

    model = BlendingClassifier()
...
def test_multi():
    ...
    assert len(model.estimators) >= 3, f'Мало базовых моделей: {len(model.estimators)}, должно быть не менее трех'
    assert model.final_estimator.n_features_in_ == 9 * len(model.estimators), f'Неверная входная размерность мета-модели. Должна быть {9 * len(model.estimators)}'
